...
class Solution:
    def batch_loader(self, raw_dataset: str, context_length: int, batch_size: int) -> Tuple[List[List[str]], List[List[str]]]:
        ...
        return X, Y
        # this is loading the data for training the gpt model uses batch loading because it can't select all the tokens to be trainined on this wouldn't be efficient